...
public class Walker{
	...
	public boolean handleObstacle(Tile[] t){
		...
		for(GameObject go : ctx.objects.select(5).name(Pattern.compile("(staircase)|(wilderness ditch)|(ladder)|((.*)?door)|(gate(.*)?)", Pattern.CASE_INSENSITIVE))){                            //Changed to filter by name else all the filtering in the next part takes much too long to be considered effective..
			...
			if(nextTile.floor() != p.tile().floor()){
				if(go.type() == GameObject.Type.INTERACTIVE && go.actions().length > 0){
					if(nextTile.floor() > p.tile().floor()){                      //Need to climb up.
						...
						System.out.println("Climbing Down " + nextTile.floor() + " " + p.tile().floor());
						if(calcDist < distance && reachable(go)){
							for(String s : go.actions()){
				...
				if(go.type() != GameObject.Type.BOUNDARY){
					for(String s : go.actions()){
						if(go.tile().distanceTo(nextTile) + go.tile().distanceTo(p) < distance && reachable(go)){
							if(s != null && !s.equals("null") && (s.contains("Climb-up") || s.contains("Climb-down") || s.contains("Enter") || s.contains("Cross"))){
								obstacle = go;
			...
			}else if(go.type() == GameObject.Type.INTERACTIVE && go.name().contains("Wilderness")){
				if(calcDist < distance && reachable(go)){
					obstacle = go;
					distance = calcDist;
				}
		...
		if(obstacle.inViewport()){
			...
			}else if(obstacle.name().equalsIgnoreCase("wilderness ditch")){
				if(obstacle.interact("Cross")){
					return handlePostInteraction();
				}
			}else{
	...
	private boolean reachable(GameObject go){
		...
		int a = go.width();
		if(go.name().equalsIgnoreCase("wilderness ditch")) a = 2;
	...
	public boolean walkPath(Tile[] t){
		...
		if(ctx.players.local().tile().floor() == 2) return false;
